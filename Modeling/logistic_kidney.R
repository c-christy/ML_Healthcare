...
train.kfoldx=model.matrix(KIQ022~., data=train)
train.kfoldy=train$KIQ022
test.kfoldx=model.matrix(KIQ022~.,data=test)
test.kfoldy=test$KIQ022
...
############## AUC ################
library(ROCR)
rocr.log.pred <- prediction(lasso.pred, test$KIQ022)
...
x = table(test$KIQ022, lasso.pred > 0.5389516)
x
