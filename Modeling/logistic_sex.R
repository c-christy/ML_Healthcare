...
prop.table(table(training$sex_y))
...
train.kfoldx=model.matrix(sex_y~., data=train)
train.kfoldy=train$sex_y
test.kfoldx=model.matrix(sex_y~.,data=test)
test.kfoldy=test$sex_y
...
############## AUC ################
library(ROCR)
rocr.log.pred <- prediction(lasso.pred, test$sex_y)
...
x = table(test$sex_y, lasso.pred > 0.4690802)
x
