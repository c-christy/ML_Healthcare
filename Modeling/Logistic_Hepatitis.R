...
train.kfoldx=model.matrix(Hepatitis~., data=train)
train.kfoldy=train$Hepatitis
test.kfoldx=model.matrix(Hepatitis~.,data=test)
test.kfoldy=test$Hepatitis
...
rocr.pred <- prediction(lasso.pred, test$Hepatitis)
...
x = table(test$Hepatitis, lasso.pred > 0.5708207)
x
